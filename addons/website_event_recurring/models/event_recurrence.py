...
class EventRecurrence(models.Model):
    ...
    # Overriding fields linked to calendar instead of event
    base_event_id = fields.Many2one(
        'event.event', ondelete='set null', copy=False)
    calendar_event_ids = fields.One2many('event.event', 'recurrence_id')

    # Overriding methods linked to calendar instead of event.
    # Creating an event from the event module, instead of an event from the calendar module
    def _apply_recurrence(self, specific_values_creation=None, no_send_edit=False, generic_values_creation=None):
        ...
        self.env['event.event'].with_context(context).create(event_vals)
